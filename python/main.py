...
class SpacetimeConsole(cmd.Cmd):
    ...
    def do_findobjs(self, line):
        ...
        name2class = fs.Store.master_dataframe.type_manager.name2class
        if len(tokens) == 3:
            type_text = tokens[0]
            dim = tokens[1]
            val = tokens[2]
    ...
    def do_descobj(self, line):
        ...
        tokens = shlex.split(line)
        name2class = fs.Store.master_dataframe.type_manager.name2class
        object_map = fs.Store.master_dataframe.object_manager.object_map
        if len(tokens) ==  2:
            type_text = tokens[0]
            oid = tokens[1]
            if type_text in name2class:
    ...
    def do_objsin(self, type_text):
        ...
        name2class = fs.Store.master_dataframe.type_manager.name2class
        if type_text in name2class:
            objs = fs.Store.get(name2class[type_text])
            if objs:
    ...
    def do_clear(self, type_text):
        ...
        if type_text:
            if type_text == "!all":
                ...
                try:
                    fs.Store.clear(name2class[type_text])
...
if __name__== "__main__":
    ...
    parser.add_argument(
        '-fl', '--flask', action='store_true', default=False,
        help='Sets up the server in flask mode. If not, Tornado is used.')
    parser.add_argument(
        '-o', '--objectless', action='store_true', default=True,
    ...
    fs = FrameServer(
        args.port, args.debug, args.timeout, args.clearempty,
        args.trackip, args.objectless)
