...
def get_request_handlers(process, store, handle_exceptions, thread_pool):
    class GetAllUpdatedTracked(BaseAllUpdatedTracked):
        ...
        def complete_update(self, sim, data, content_type):
            try:
                ...
                process.logger.info("completing get from %s", sim)
            except Exception, e:
        ...
        def _complete_update(self, sim, data, content_type):
            try:
                ...
                process.logger.info("completing get from %s", sim)
                self.finish()
                process.logger.info("completed get from %s", sim)
            except Exception, e:
                print e, e.__class__.__name__
    ...
    class PostAllUpdatedTracked(BaseAllUpdatedTracked):
        ...
        def complete_push(self, sim):
            try:
                ...
                process.logger.info("completing push from %s", sim)
            except Exception, e:
                print e, e.__class__.__name__
    ...
    class Register(BaseRegisterHandler):
        @handle_exceptions
        def put(self, sim):
            process.logger.info("Register from %s", sim)
            data = self.request.body
...
class TornadoServerProcess(Thread):
    ...
    def process_start(self, req):
        ...
        self.port = req.port
        self.store.start()
        self.logger.info("Started the store.")
        if req.console:
        ...
        self.logger.info("Bound the port")
